  ...
  return (
    <nav className="w-full flex py-6 justify-between item-center navbar">
      ...
      <ul className="list-none sm:flex justify-end items-center flex-1">
        <li className={`font-poppins font-normal cursor-pointer text-[16px]  text-white  text-dimWhite mr-10`}>
        ...
        >
          <ul className="list-none sm:flex justify-end items-center flex-1 flex-col">
            {navLinks.map((nav, index) => (
              <li
                key={nav.id}
                className={`font-poppins font-normal cursor-pointer text-[16px] text-white text-dimWhite ${
                  index === navLinks.length - 1 ? "mb-4" : "mr-10"
                }`}
              >
                <a href={"/"}> {nav.title} </a>
              </li>
